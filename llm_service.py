...
1. 基本面正向变动强度（Fundamental_Positive）  
   ...
   - 取值0.5：中性或不相干影响，如“未有任何影响公司营收利润”“市场对公司产品的评价存在争议”。 
   - 中间值示例：0.3（“有负面影响，但影响范围有限，不会对营收构成重大影响”）；0.8（“成本端原材料价格下降，预计年度利润提升10%”）。

2. 影响周期长度（Impact_Cycle_Length）
   - 定义：新闻事件对公司的影响持续时间。
   - 取值1：长期影响（如“签订5年独家供应协议”“发布未来3年战略规划，投入50亿研发”）。
   - 取值0：短期影响（如“公司获得1000万短期政府补贴”“单日促销活动，预计增销1000万”）。
   - 中间值：0.7（“发布新款产品，预计贡献未来1年营收”）、0.5（“影响幅度为半年”）、0.3（“影响范围为一个季度”）。


3. 事件时效性权重（Timeliness_Weight）  
   - 定义：新闻事件的即时性与紧迫性（突发/近期事件权重高，过时信息权重低），通过对比**新闻内容中的时间描述**相对于**发布时间**的时间差。  
   - 取值1：突发重大即时事件，如“今日盘中突发：公司获得国家级专项补贴5亿元”“半小时前公告：重大合同签署生效”。  
   - 取值0：过时或滞后信息，如“回顾半年前公司的战略调整（已被市场消化）”“相对于发布时间3个月前的行业分析报告（无新信息）”。  
   - 中间值示例：0.8（“昨日晚间公告的季度业绩，尚未被充分交易”）；0.3（“相对于发布时间一周前的产品发布会”）。
...
5. 信息相关度（Information_Relevance）  
   ...
   - 中间值示例：0.6（“新闻提及目标公司子公司的小额合作项目，不涉及母公司核心业务”）；0.2（“行业报告中列举目标公司为行业参与者之一，无具体信息”）。
